...
function addParagraph() {
    ...
    function addActionForm(paragraphIndex) {
        ...
        var actionHtml = `
            <div class="action-form">
                <div class="form-group">
                    ...
                    <select id="action-type-${paragraphIndex}" name="paragraphs[${paragraphIndex}][actions][][type]" class="form-control">
        ...
        const $nextParagraphDropdown = $actionsContainer.find('.action-next-paragraph-dropdown').last();
        updateNextParagraphOptions($nextParagraphDropdown);
    
        // Сразу вызываем обновление видимости в зависимости от типа действия
        const $actionTypeSelect = $actionsContainer.find(`#action-type-${paragraphIndex}`);
        updateActionTypeVisibility($actionTypeSelect.val(), $actionsContainer.find('.action-form').last());
        initSingleChoiceVisibility($actionsContainer.find('.action-form').last());  // Передаем только добавленную форму

        // Слушаем изменения в выпадающем списке "Тип действия"
        $actionTypeSelect.on('change', function() {
            var actionType = $(this).val();
            updateActionTypeVisibility(actionType, $actionsContainer.find('.action-form').last());
        });
    
        // Обработчик для кнопки добавления элемента в левую группу
        $actionsContainer.on('click', `.add-matching-left`, function() {
        ...
        $actionsContainer.on('click', `.add-matching-right`, function() {
        ...
        // Обработчик для галочки "Правильное сочетание"
        $actionsContainer.on('change', `.mark-correct`, function() {
        ...
        // Обработчик для кнопки "Добавить условие перехода"
        $actionsContainer.on('click', `.add-transition-condition`, function() {
        ...
        // Обработчик для кнопки добавления варианта в выборе нескольких вариантов
$actionsContainer.on('click', `.add-multiple-choice-option`, function() {
...
// Обработчик для изменения состояния галочки "Правильный ответ"
$actionsContainer.on('change', '.correct-option', function() {
    var $checkbox = $(this);
...
$actionsContainer.on('click', '.remove-option', function() {
    $(this).closest('.form-group').remove();
});

        // Удаление условия перехода
        $actionsContainer.on('click', `.remove-transition-condition`, function() {
